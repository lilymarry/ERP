...
-(void)setBadgeValue:(NSInteger)value {
  if(value != 0 || self.displayWhenZero) {
    CGSize numberSize = [[NSString stringWithFormat:@"%ld", (long)value] sizeWithFont:self.font];
    ...
    //self.center = (self.placement == kBadgePlacementUpperLeft) ? CGPointMake(0, 0) : CGPointMake(superviewFrame.size.width, 0);
      self.center = (self.placement == kBadgePlacementUpperLeft) ? CGPointMake(0, 0) : CGPointMake(superviewFrame.size.width-self.bounds.size.width-5, self.bounds.size.height);
  } else {
...
-(void)drawRect:(CGRect)rect {
  if(self.badgeValue != 0 || self.displayWhenZero) {
    ...
    [self.textColor set];
    CGSize numberSize = [[NSString stringWithFormat:@"%ld", (long)self.badgeValue] sizeWithFont:self.font];
    [[NSString stringWithFormat:@"%ld", (long)self.badgeValue] drawInRect:CGRectMake(self.outlineWidth + 3, (rect.size.height / 2.0) - (numberSize.height / 2.0), rect.size.width - (self.outlineWidth * 2) - 6, numberSize.height) withFont:self.font lineBreakMode:UILineBreakModeClip alignment:UITextAlignmentCenter];
    }
}
